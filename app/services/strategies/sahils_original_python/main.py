...
def construct_prompt(question: str, context_embeddings: dict, document_pages: pd.DataFrame):
    """
    ...
    for _, section_index in most_relevant_document_sections:
        document_section = document_pages.loc[document_pages['title']
                                              == section_index].iloc[0]

        chosen_sections_len += document_section.numtokens + separator_len
...
def ask_a_question(question_asked):
    path = "common_knowledge/minimalist_entrepreneur"
    document_pages = pd.read_csv(f'{path}.pages.csv')
    document_embeddings = load_embeddings(f'{path}.pdf.embeddings.csv')
    answer, context = answer_query_with_context(
        question_asked, document_pages, document_embeddings)
    # return {"question": question_asked, "answer": answer, "context": context}
    return answer


# print(ask_a_question("What does the author think about remote work"))
# print(ask_a_question("What does being successful mean in SF?"))
# print(ask_a_question("Why do businesses fail according to author?"))
